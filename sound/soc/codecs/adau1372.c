...
#define ADAU1372_SAI0_RJ24			(0x2 << 6)
#define ADAU1372_SAI0_RJ16			(0x3 << 6)
...
static const DECLARE_TLV_DB_SCALE(adau1372_pga_tlv, -1200, 75, 0);//用于定义一个db值映射的tlv_array.
static const DECLARE_TLV_DB_SCALE(adau1372_pga_boost_tlv, 0, 1000, 0);
/*
adau1372_pga_tlv的类型是DECLARE_TLV_DB_SCALE，reg最小值-12db,/寄存器每增加一个单位，对应增加0.75db
ADC 0 Capture Volume控件：min0x00:-12db,max0xff:
*/
...
static const struct snd_kcontrol_new adau1372_controls[] = {// xname, reg, shift, max, invert, tlv
	SOC_SINGLE_TLV("ADC 0 Capture Volume", ADAU1372_REG_ADC_VOL(0),//tlv:描述reg设定值，与他代表的实际意义之间的映射关系
		0, 0xff, 1, adau1372_digital_tlv),                         //例如：音量控件：设定值对应的db之间的映射关系
...
static const struct snd_soc_dapm_widget adau1372_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("MICBIAS0", ADAU1372_REG_MICBIAS, 4, 1, NULL, 0),
	SND_SOC_DAPM_SUPPLY("MICBIAS1", ADAU1372_REG_MICBIAS, 5, 1, NULL, 0),

	SND_SOC_DAPM_PGA("PGA0", ADAU1372_REG_PGA_CTRL(0), 7, 1, NULL, 0),
	SND_SOC_DAPM_PGA("PGA1", ADAU1372_REG_PGA_CTRL(1), 7, 1, NULL, 0),
	SND_SOC_DAPM_PGA("PGA2", ADAU1372_REG_PGA_CTRL(2), 7, 1, NULL, 0),
	SND_SOC_DAPM_PGA("PGA3", ADAU1372_REG_PGA_CTRL(3), 7, 1, NULL, 0),
//	SND_SOC_DAPM_ADC("ADC0", NULL, ADAU1372_REG_ADC_CTRL2, 0, 0),
	SND_SOC_DAPM_ADC("ADC0", NULL, ADAU1372_REG_ADC_CTRL2, 1, 0),
	SND_SOC_DAPM_ADC("ADC1", NULL, ADAU1372_REG_ADC_CTRL2, 1, 0),
//	SND_SOC_DAPM_ADC("ADC2", NULL, ADAU1372_REG_ADC_CTRL3, 0, 0),
	SND_SOC_DAPM_ADC("ADC2", NULL, ADAU1372_REG_ADC_CTRL3, 1, 0),
...
//		NULL, 0),
	SND_SOC_DAPM_SUPPLY("Input ASRC Supply", ADAU1372_REG_ASRC_MODE, 1, 0,
		NULL, 0),
...
//	SND_SOC_DAPM_DAC("DAC0", NULL, ADAU1372_REG_DAC_CTRL, 0, 0),
	SND_SOC_DAPM_DAC("DAC0", NULL, ADAU1372_REG_DAC_CTRL, 3, 0),
	SND_SOC_DAPM_DAC("DAC1", NULL, ADAU1372_REG_DAC_CTRL, 3, 0),
...
#if 0
static unsigned int adau1372_reg_read(struct regmap *map, unsigned int reg)
{
	unsigned int val;
...
#endif
#if 1
static void adau1372_reg_update_enable(struct regmap *map, bool enable)
{
	unsigned int val;

	regmap_read(map, ADAU1372_REG_CLK_CTRL, &val);

	if ((enable == true) && !(val & 0x01))
		regmap_update_bits(map, ADAU1372_REG_CLK_CTRL, ADAU1372_CLK_CTRL_MCLK_EN, 0x1);
#if defined(CONFIG_ARCH_CVITEK)
#else
	else if ((enable == false) && (val & 0x01))
		regmap_update_bits(map, ADAU1372_REG_CLK_CTRL, ADAU1372_CLK_CTRL_MCLK_EN, 0x0);
#endif
};
#endif
static int adau1372_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	dev_dbg(adau1372->dev, "%s, dai_name=%s, fmt=0x%08x\n", __func__, dai->name, fmt);
#if defined(CONFIG_SND_SOC_CV1835_CONCURRENT_I2S)
	dev_dbg(adau1372->dev, "set codec to SLAVE mode\n");
#else
	switch (fmt & SND_SOC_DAIFMT_MASTER_MASK) {
	case SND_SOC_DAIFMT_CBM_CFM:
		dev_dbg(adau1372->dev, "set codec to MASTER mode\n");
	...
	case SND_SOC_DAIFMT_NB_NF:
		dev_dbg(adau1372->dev, "set codec to NB_NF\n");
#if defined(CONFIG_ARCH_CVITEK)
	...
	case SND_SOC_DAIFMT_NB_IF:
		dev_dbg(adau1372->dev, "set codec to NB_IF\n");
#if defined(CONFIG_ARCH_CVITEK)
		sai1 |= ADAU1372_SAI1_BCLKEDGE;
#endif
		invert_lrclk = true;
	...
	case SND_SOC_DAIFMT_IB_NF:
		dev_dbg(adau1372->dev, "set codec to IB_NF\n");
		invert_lrclk = false;
		break;
	case SND_SOC_DAIFMT_IB_IF:
		dev_dbg(adau1372->dev, "set codec to IB_IF\n");
		invert_lrclk = true;
		break;
	}

	regmap_read(adau1372->regmap, ADAU1372_REG_SAI0, &sai0);

	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
	case SND_SOC_DAIFMT_PDM:
		dev_dbg(adau1372->dev, "set codec to I2S mode\n");
		sai0 |= ADAU1372_SAI0_DELAY1;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		dev_dbg(adau1372->dev, "set codec to LEFT_J mode\n");
		sai0 |= ADAU1372_SAI0_DELAY0;
		invert_lrclk = !invert_lrclk;
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		dev_dbg(adau1372->dev, "set codec to RIGHT_J mode\n");
		sai0 |= ADAU1372_SAI0_RJ24;
		invert_lrclk = !invert_lrclk;
		break;
	case SND_SOC_DAIFMT_DSP_A:
		dev_dbg(adau1372->dev, "set codec to DSP_A mode\n");
		sai0 |= ADAU1372_SAI0_DELAY1;
		sai1 |= ADAU1372_SAI1_LR_MODE;
		break;
	case SND_SOC_DAIFMT_DSP_B:
		dev_dbg(adau1372->dev, "set codec to DSP_B mode\n");
		sai0 |= ADAU1372_SAI0_DELAY0;
		sai1 |= ADAU1372_SAI1_LR_MODE;
		break;
	}
	...
	adau1372_reg_update_enable(adau1372->regmap, true);
	...
	adau1372_reg_update_enable(adau1372->regmap, false);
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	dev_dbg(adau1372->dev, "set slot_width = %d\n", slot_width);

	switch (slot_width) {
	case 16:
		sai1 = ADAU1372_SAI1_BCLKRATE | ADAU1372_SAI1_BCLK_TDMC;
		break;
	case 24:
		sai1 = 0;
		break;
	...
	adau1372_reg_update_enable(adau1372->regmap, true);
	regmap_update_bits(adau1372->regmap, ADAU1372_REG_SAI0,
		ADAU1372_SAI0_SAI_MASK | ADAU1372_SAI0_FS_MASK, sai0);
	regmap_update_bits(adau1372->regmap, ADAU1372_REG_SAI1,
		ADAU1372_SAI1_BCLKRATE | ADAU1372_SAI1_BCLK_TDMC, sai1);
	adau1372_reg_update_enable(adau1372->regmap, false);
	...
	enum snd_soc_bias_level level)
{
	...
	dev_dbg(adau1372->dev, "%s, level=%d\n", __func__, level);
	switch (level) {
	case SND_SOC_BIAS_ON:
		regmap_write(adau1372->regmap, ADAU1372_REG_ASRC_MODE, 0x03);
		regmap_write(adau1372->regmap, ADAU1372_REG_ADC_CTRL2, 0x03);
		regmap_write(adau1372->regmap, ADAU1372_REG_ADC_CTRL3, 0x03);
		regmap_write(adau1372->regmap, ADAU1372_REG_DAC_CTRL, 0x03);
		regmap_write(adau1372->regmap, ADAU1372_REG_OP_STAGE_CTRL, 0x30);
		regmap_write(adau1372->regmap, ADAU1372_REG_INTERP_PWR, 0x0f);
		break;
...
static void adau1372_reg_init(struct regmap *regmap)
{
	regmap_write(regmap, ADAU1372_REG_DAC_SOURCE, 0xDC);
#if defined(CONFIG_SND_SOC_CV1835_USE_AUDIO_PLL)
	regmap_write(regmap, ADAU1372_REG_CLKOUT_SEL, 0x07); /* CLOCK OUTPUT OFF*/
#else
	regmap_write(regmap, ADAU1372_REG_CLKOUT_SEL, 0x01); /* CLOCK OUT */
#endif

#if defined(CONFIG_ARCH_CV183X_FPGA)
	regmap_write(regmap, ADAU1372_REG_SOUT_SOURCE_0_1, 0x76); /* Set AIN3/2 as audio input source */
#else
	regmap_write(regmap, ADAU1372_REG_SOUT_SOURCE_0_1, 0x55); /* Set AIN1/1 as audio input source */
#endif
	regmap_write(regmap, ADAU1372_REG_SOUT_SOURCE_2_3, 0x44);
	regmap_write(regmap, ADAU1372_REG_SOUT_SOURCE_4_5, 0x44);
	regmap_write(regmap, ADAU1372_REG_SOUT_SOURCE_6_7, 0x44);

	regmap_write(regmap, ADAU1372_REG_ASRCO_SOURCE_0_1, 0x54);
	regmap_write(regmap, ADAU1372_REG_ASRCO_SOURCE_2_3, 0x76);
	regmap_write(regmap, ADAU1372_REG_ASRC_MODE, 0x03);
	regmap_write(regmap, ADAU1372_REG_ADC_CTRL0, 0x01);
	regmap_write(regmap, ADAU1372_REG_ADC_CTRL1, 0x01);
	regmap_write(regmap, ADAU1372_REG_ADC_CTRL2, 0x03);
	regmap_write(regmap, ADAU1372_REG_ADC_CTRL3, 0x03);
	regmap_write(regmap, ADAU1372_REG_MICBIAS, 0x20);
	regmap_write(regmap, ADAU1372_REG_DAC_CTRL, 0x03);
	regmap_write(regmap, ADAU1372_REG_OP_STAGE_MUTE, 0x00);
	regmap_write(regmap, ADAU1372_REG_SAI0, 0x10);
	regmap_write(regmap, ADAU1372_REG_SAI1, 0x00);
	regmap_write(regmap, ADAU1372_REG_SOUT_CTRL, 0xfc);
	regmap_write(regmap, ADAU1372_REG_MODE_MP(1), 0x00);
	regmap_write(regmap, ADAU1372_REG_MODE_MP(4), 0x11);
	regmap_write(regmap, ADAU1372_REG_MODE_MP(5), 0x10);
#if defined(CONFIG_SND_SOC_CV1835_USE_AUDIO_PLL)
	regmap_write(regmap, ADAU1372_REG_MODE_MP(6), 0x11); /* PUSH DOWN */
#else
	regmap_write(regmap, ADAU1372_REG_MODE_MP(6), 0x12); /* CLOCKOUT */
#endif
	regmap_write(regmap, ADAU1372_REG_PB_VOL_SET, 0x10);
	regmap_write(regmap, ADAU1372_REG_PB_VOL_CONV, 0x87);
	regmap_write(regmap, ADAU1372_REG_DEBOUNCE_MODE, 0x03);
	regmap_write(regmap, ADAU1372_REG_OP_STAGE_CTRL, 0x30);
	regmap_write(regmap, ADAU1372_REG_DECIM_PWR, 0xff);
	regmap_write(regmap, ADAU1372_REG_INTERP_PWR, 0x0f);
	regmap_write(regmap, ADAU1372_REG_DAC_VOL(0), 0x10);
	regmap_write(regmap, ADAU1372_REG_DAC_VOL(1), 0x10);
}
	...
	void (*switch_mode)(struct device *dev))
{
	...
	adau1372->regmap->cache_bypass = true;
