...
struct _dma_dbg_i {
	...
	uint32_t		wdma_2_err_sts;
	uint32_t		wdma_2_idle;
	uint32_t		wdma_3_err_sts;
	uint32_t		wdma_3_idle;
	uint32_t		rdma_0_err_sts;
	uint32_t		rdma_0_idle;
	uint32_t		rdma_1_err_sts;
	uint32_t		rdma_1_idle;
};
...
struct _isp_cfg {
	...
	enum ISP_YUV_SCENE	yuv_scene_mode;
	enum ISP_TNR_TYPE	tnr_mode;
	...
	uint32_t		is_raw_replay_fe	: 1;
	uint32_t		is_raw_replay_be	: 1;
	uint32_t		is_yuv_sensor		: 1;
	uint32_t		is_422_to_420		: 1;
	uint32_t		is_bt_demux		: 1;
	...
	uint32_t		is_postout_crop		: 1;
};

struct _isp_bind_info {
	bool			is_bind;
	uint8_t			bind_dev_num;
};
/*
...
void ispblk_lmap_chg_size(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num, const enum cvi_isp_fe_chn_num chn_num);
 ...
 *	RAW TOP SUBSYS
 ****************************************************************************/
void ispblk_bnr_config(struct isp_ctx *ctx, enum ISP_BLK_ID_T blk_id, enum ISP_BNR_OUT out_sel,
...
void ispblk_rgbcac_config(struct isp_ctx *ctx, enum ISP_BLK_ID_T blk_id, bool en, uint8_t test_case);
void ispblk_lcac_config(struct isp_ctx *ctx, enum ISP_BLK_ID_T blk_id, bool en, uint8_t test_case);
...
void vi_tuning_clut_update(
	struct isp_ctx *ctx,
	enum cvi_isp_raw raw_num);
int vi_tuning_get_clut_tbl_idx(
	enum cvi_isp_raw raw_num,
	int tun_idx);
...
void ispblk_rgbir_tun_cfg(
	struct isp_ctx *ctx,
	struct cvi_vip_isp_rgbir_config *cfg,
	const enum cvi_isp_raw raw_num);
/****************************************************************************
