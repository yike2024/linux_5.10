...
struct vi_clut_idx gClutIdx[ISP_PRERAW_MAX];
...
int vi_tuning_sw_init(void)
{
	...
	for (i = 0; i < ISP_PRERAW_MAX; i++) {
		for (j = 0; j < TUNING_NODE_NUM; j++) {
			gClutIdx[i].clut_tbl_idx[j] = 0;
		}
		spin_lock_init(&gClutIdx[i].clut_idx_lock);
	}
...
int vi_tuning_buf_setup(struct isp_ctx *ctx)
{
	...
	for (i = 0; i < ISP_PRERAW_MAX; i++) {
		...
		be_paddr = phyAddr + VI_ALIGN(sizeof(struct cvi_vip_isp_post_cfg));
		tuning_buf_addr.be_addr[dev_num] = be_paddr;
		tuning_buf_addr.be_vir[dev_num] = phys_to_virt(be_paddr);

		fe_paddr = phyAddr + VI_ALIGN(sizeof(struct cvi_vip_isp_post_cfg))
				+ VI_ALIGN(sizeof(struct cvi_vip_isp_be_cfg));
...
void vi_tuning_buf_clear(void)
{
	...
	for (i = 0; i < ISP_PRERAW_MAX; i++) {
		...
		if (tuning_buf_addr.post_vir[i] != NULL) {
			...
			tun_idx = post_cfg->tun_idx;
			vi_pr(VI_DBG, "Clear post tuning tun_update(%d), tun_idx(%d)\n",
		...
		if (tuning_buf_addr.be_vir[i] != NULL) {
			...
			vi_pr(VI_DBG, "Clear be tuning tun_update(%d), tun_idx(%d)\n",
		...
		if (tuning_buf_addr.fe_vir[i] != NULL) {
			...
			vi_pr(VI_DBG, "Clear fe tuning tun_update(%d), tun_idx(%d)\n",
	...
	enum cvi_isp_raw raw_num)
{
	...
	fe_cfg = (struct cvi_vip_isp_fe_cfg *)tuning_buf_addr.fe_vir[dev_num];
	tun_idx = fe_cfg->tun_idx;
	...
	if (fe_tun->bnr.update) {
		ctx->ai_bnr_cfg.blc = fe_tun->bnr.blc;
		ctx->ai_bnr_cfg.coeff_a = fe_tun->bnr.coeff_a;
		ctx->ai_bnr_cfg.coeff_b = fe_tun->bnr.coeff_b;
		ctx->ai_bnr_cfg.filter_motion_str_2d = fe_tun->bnr.filter_motion_str_2d;
		ctx->ai_bnr_cfg.filter_static_str_2d = fe_tun->bnr.filter_static_str_2d;
		ctx->ai_bnr_cfg.filter_str_3d = fe_tun->bnr.filter_str_3d;
	}
	...
	enum cvi_isp_raw raw_num)
{
	...
	for (idx = 0; idx < 2; idx++) {
		struct cvi_vip_isp_blc_config	*blc_cfg;
		struct cvi_vip_isp_rgbir_config	*rgbir_cfg;
