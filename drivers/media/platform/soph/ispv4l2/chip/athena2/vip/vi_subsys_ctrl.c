...
void ispblk_splt_config(struct isp_ctx *ctx, enum cvi_isp_raw raw_num, bool enable)
{
	...
	if (ctx->isp_pipe_cfg[raw_num].is_tile) {
		...
		// left
		ISP_WR_BITS(ba, REG_ISP_LINE_SPLITER_T, IMG_SIZE_FE0, IMG_IN_W_M1, width_l - 1);
		ISP_WR_BITS(ba, REG_ISP_LINE_SPLITER_T, IMG_SIZE_FE0, IMG_IN_H_M1, height - 1);
...
void ispblk_isptop_config(struct isp_ctx *ctx)
{
	...
	//line_spliter intr ctrl, when multi sensor ,close intr enable
	ev0_en_fe345.bits.LINE_SPLITER_DMA_DONE_ENABLE_FE0	= ctx->is_multi_sensor ? 0 : 0xF;
	ev0_en_fe345.bits.LINE_SPLITER_DMA_DONE_ENABLE_FE1	= ctx->is_multi_sensor ? 0 : 0xF;
	ev2_en.bits.FRAME_ERR_LINE_SPLITER_ENABLE_FE0		= ctx->is_multi_sensor ? 0 : 0x1;
	ev2_en.bits.FRAME_ERR_LINE_SPLITER_ENABLE_FE1		= ctx->is_multi_sensor ? 0 : 0x1;
