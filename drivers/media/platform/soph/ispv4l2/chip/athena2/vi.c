...
static struct cvi_vip_fmt cvi_vip_formats[] = {
	{
		...
		dma_addr_t *dma_handle, gfp_t gfp, unsigned long attrs)
{
	...
	mutex_lock(&gvidev->stream_lock);
	index = vb2_buf_mem_index++;
	mutex_unlock(&gvidev->stream_lock);
			...
			dma_addr_t dma_handle, unsigned long attrs)
{
	...
	vi_pr(VI_INFO, "global_index:%d, dma_fd[%d]:%d\n", vb2_buf_mem_index, index, dmabuf_fd);
}
...
static void _vi_dma_free(struct cvi_vi_dev *videv, struct mem_mapping *mem_info)
{
	...
	if (!mem_info->phy_addr) {
		vi_pr(VI_INFO, "NULL mem to free!\n");
		return;
	}
...
static int vi_mempool_alloc_ion(struct cvi_vi_dev *videv)
{
	...
	for (i = 0; i < ISP_PRERAW_MAX; i++) {
		ret = _vi_dma_alloc(videv, &vi_cmdq_buf_info[i],
		&ctx->isp_pipe_cfg[i].cmdq_buf.phy_addr, VI_CMDQ_BUF_SIZE, true, "VI_CMDQ_BUF");
		if (ret) {
			vi_pr(VI_ERR, "alloc cmdq ion fail! ret(%d)\n", ret);
			_vi_dma_free(videv, &vi_dma_buf_info);
			return ret;
		}
		ctx->isp_pipe_cfg[i].cmdq_buf.buf_size = VI_CMDQ_BUF_SIZE;
		ctx->isp_pipe_cfg[i].cmdq_buf.vir_addr = vi_cmdq_buf_info[i].vir_addr;
	}
...
static void vi_mempool_free_ion(struct cvi_vi_dev *videv)
{
	...
	for (i = 0; i < ISP_PRERAW_MAX; i++) {
		_vi_dma_free(videv, &vi_cmdq_buf_info[i]);
	}
}
...
static void _isp_yuv_bypass_buf_enq(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num, const u8 buf_chn)
{
	...
	vi_pr(VI_INFO, "update chn[%d] yuv bypass outbuf: 0x%llx\n", buf_chn, tmp_addr);
...
void _vi_ctrl_init(enum cvi_isp_raw raw_num, struct cvi_vi_dev *vdev)
{
	...
	if (vdev->snr_info[raw_num].snr_fmt.img_size[0].active_w != 0) { //MW config snr_info flow
		...
		if (vdev->snr_info[raw_num].snr_fmt.frm_num > 1) { //HDR
			ictx->isp_pipe_cfg[raw_num].crop_se.w = vdev->snr_info[raw_num].snr_fmt.img_size[1].active_w;
			ictx->isp_pipe_cfg[raw_num].crop_se.h = vdev->snr_info[raw_num].snr_fmt.img_size[1].active_h;
			ictx->isp_pipe_cfg[raw_num].crop_se.x = vdev->snr_info[raw_num].snr_fmt.img_size[1].start_x;
			ictx->isp_pipe_cfg[raw_num].crop_se.y = vdev->snr_info[raw_num].snr_fmt.img_size[1].start_y;
	...
	} else if (ictx->isp_pipe_cfg[raw_num].is_raw_replay_fe ||
...
void _vi_scene_ctrl(struct cvi_vi_dev *vdev)
{
	...
	if (gViCtx->total_dev_num >= 2) { //Multi sensor scenario
		...
		if (!ctx->isp_pipe_cfg[raw_num].is_yuv_sensor) { // rgb sensor
			if (ctx->isp_pipe_cfg[raw_num].is_raw_replay_fe) {
				if (vdev->usr_fmt.width > 4608 || tile_en) {
					ctx->is_tile = true;
					ctx->is_fbc_on = false;
					ctx->isp_pipe_cfg[raw_num].is_tile = true;
					ctx->isp_pipe_cfg[raw_num + 1].is_tile = true;
				}
				vi_pr(VI_INFO, "fe rawreplay is_tile[%d], tnr_mode[%d]\n",
						ctx->is_tile, ctx->isp_pipe_cfg[raw_num].tnr_mode);
			}
		}
#endif
		...
		vi_pr(VI_INFO, "is_3dnr_on[%d], is_fbc_on[%d], is_tile[%d], is_tnr_ai_isp[%d], is_raw_ai_isp[%d]\n",
				ctx->is_3dnr_on, ctx->is_fbc_on, ctx->is_tile,
				ctx->isp_pipe_cfg[raw_num].is_tnr_ai_isp,
				ctx->isp_pipe_cfg[raw_num].is_raw_ai_isp);
	}
...
static void _vi_splt_set_all_state(
	...
	const enum cvi_isp_state splt_state)
{
	struct isp_ctx *ctx = &vdev->ctx;
...
int vi_start_streaming(struct cvi_vi_dev *vdev)
{
	...
	for (raw_num = ISP_PRERAW0; raw_num < ISP_PRERAW_MAX; raw_num++) {
		...
		if (!vdev->ctx.isp_pipe_cfg[raw_num].is_raw_replay_fe &&
		    ...
		    !vdev->ctx.isp_pipe_cfg[raw_num].is_raw_ai_isp) {
	...
	if (_is_fe_be_online(&vdev->ctx)) {
		...
		if (!vdev->ctx.isp_pipe_cfg[raw_num].is_raw_replay_be &&
			...
			if (vdev->ctx.isp_pipe_cfg[raw_num].is_raw_ai_isp ||
			    vdev->ctx.isp_pipe_cfg[raw_num].is_tile ||
			    line_spliter_en) {
...
static int _pre_fe_inbuf_enque(
	struct cvi_vi_dev *vdev,
	const enum cvi_isp_raw raw_num,
	const enum cvi_isp_fe_chn_num chn_num)
{
	...
	struct isp_buffer *b = NULL;
	uint64_t splt_rdma_addr = 0;
	uint32_t splt_fe0_r = (chn_num == ISP_FE_CH0) ?
				ISP_BLK_ID_DMA_CTL_SPLT_FE0_RDMA_LE : ISP_BLK_ID_DMA_CTL_SPLT_FE0_RDMA_SE;
	uint32_t splt_fe1_r = (chn_num == ISP_FE_CH0) ?
				ISP_BLK_ID_DMA_CTL_SPLT_FE1_RDMA_LE : ISP_BLK_ID_DMA_CTL_SPLT_FE1_RDMA_SE;
...
static void _splt_hw_enque(struct cvi_vi_dev *vdev, const enum cvi_isp_raw hw_raw_num)
{
	...
	struct isp_queue *splt_r_q = NULL;
	uint32_t splt_fe0_w, splt_fe0_r;
	uint32_t splt_fe1_w, splt_fe1_r;
	enum cvi_isp_fe_chn_num chn_num, chn_max;
	enum cvi_isp_raw raw_num = hw_raw_num;
	...
	if (ctx->isp_pipe_cfg[raw_num].is_raw_replay_fe) {
		...
		for (chn_num = ISP_FE_CH0; chn_num <= chn_max; chn_num++) {
			...
			splt_fe0_r = (chn_num == ISP_FE_CH0) ?
					ISP_BLK_ID_DMA_CTL_SPLT_FE0_RDMA_LE : ISP_BLK_ID_DMA_CTL_SPLT_FE0_RDMA_SE;
			splt_fe1_w = (chn_num == ISP_FE_CH0) ?
					ISP_BLK_ID_DMA_CTL_SPLT_FE1_WDMA_LE : ISP_BLK_ID_DMA_CTL_SPLT_FE1_WDMA_SE;
			splt_fe1_r = (chn_num == ISP_FE_CH0) ?
					ISP_BLK_ID_DMA_CTL_SPLT_FE1_RDMA_LE : ISP_BLK_ID_DMA_CTL_SPLT_FE1_RDMA_SE;

			// set wdma address
			b = isp_buf_next(splt_w_q);
			if (!b) {
				vi_pr(VI_WARN, "splt_out_%d chn_num_%d is empty, use previous wdma addr.\n",
						raw_num, ISP_FE_CH0);
	...
	const enum cvi_isp_fe_chn_num chn_num)
{
	...
	if (_is_fe_be_online(ctx) || _is_all_online(ctx)) { //fe->be->dram->post or on the fly mode
		...
		be_in_q = (chn_num == ISP_FE_CH0) ?
				&pre_be_in_q : &pre_be_in_se_q[raw_num];
...
static void _v4l2_init_config_info(struct cvi_vi_dev *vdev, u8 ViDev, u8 raw_num)
{
	...
	vdev->ctx.rgb_color_mode[snr_info.raw_num]	= ISP_BAYER_TYPE_GB;
	...
	vdev->pipe[raw_num].sizeimage[1] = vdev->pipe[raw_num].bytesperline[1] * *crop_h / 2;
}
	...
	struct v4l2_format *f)
{
	...
	pix->pixelformat  = videv->fmt->fourcc;
				...
				struct v4l2_format *f)
{
	...
	crop_w = videv->ctx.isp_pipe_cfg[raw_num].post_img_w;
	crop_h = videv->ctx.isp_pipe_cfg[raw_num].post_img_h;
	videv->pipe[raw_num].bytesperline[0] = videv->pipe[raw_num].bytesperline[1] = crop_w;
	videv->pipe[raw_num].sizeimage[0] = videv->pipe[raw_num].bytesperline[0] * crop_h;
	videv->pipe[raw_num].sizeimage[1] = videv->pipe[raw_num].bytesperline[1] * crop_h / 2;
...
int cvi_isp_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	int ret = 0;
	int index = 0;
	int timeout = 300; // base timeout ms
	u8 chn_id = get_video_index(vdev);
	u8 raw_num = vi_get_raw_num_by_dev(&videv->ctx, chn_id);

	WARN_ON(!videv);

	vi_pr(VI_INFO, "dev_%d dq pipe_%d start\n", chn_id, raw_num);

	if (gViCtx->bypass_frm[raw_num] > videv->postraw_frame_number[raw_num])
		timeout += gViCtx->bypass_frm[raw_num] * 200;
	...
	vi_pr(VI_INFO, "dev_%d dqbuf at postframe[%d] vb_index[%d] done\n",
			      ...
			      unsigned int sizes[], struct device *alloc_devs[])
{
	...
	unsigned int planes = videv->fmt->buffers;
	unsigned int p;
...
static void cvi_isp_buf_queue_vb2(struct vb2_buffer *vb)
{
	...
	vi_pr(VI_INFO, "dev_%d queue vb[%d] at pipe_%d, total_buf_num[%d]\n",
	...
	// 	if (videv->qbuf_num[raw_num] == 1 && (vb2_is_streaming(vq)))
	// 		//TODO
	// 		_isp_yuv_bypass_trigger(videv, 0, raw_num);
	// } else {
	// 	if (videv->qbuf_num[raw_num] == 1 && (vb2_is_streaming(vq)))
	// 		_post_hw_enque(videv);
	// }
}
