...
extern int vi_register_video_device(struct cvi_isp_device *isp_dev);
extern int vi_destroy_instance(struct cvi_isp_device *isp_dev);

/********************* ispsubdev function ***********************/
extern int isp_subdev_register(struct cvi_isp_device *isp_dev);
extern void isp_subdev_unregister(struct cvi_isp_device *isp_dev);

/************************* media controller ***************************/
static int cvi_isp_create_links(struct cvi_isp_device *dev)
{
	unsigned int s, pad;
	int ret = 0;
	...
	for (s = 0; s < dev->num_sensors; ++s) {
		struct cvi_sensor_info *sensor = &dev->sensors[s];
...
static int subdev_notifier_complete(struct v4l2_async_notifier *notifier)
{
	...
	dev = container_of(notifier, struct cvi_isp_device, notifier);
	v4l2_dev = &dev->v4l2_dev;

	v4l2_info(v4l2_dev, "Async subdev notifier start\n");

	mutex_lock(&dev->media_dev.graph_mutex);
	ret = cvi_isp_create_links(dev);
	if (ret < 0) {
...
{
	struct cvi_isp_device *isp_dev = container_of(notifier,
					struct cvi_isp_device, notifier);
	struct cvi_isp_async_subdev *s_asd = container_of(asd,
					struct cvi_isp_async_subdev, asd);
...
static int cvi_isp_fwnode_parse(struct device *dev,
			       struct v4l2_fwnode_endpoint *vep,
			       struct v4l2_async_subdev *asd)
{
	struct cvi_isp_async_subdev *cvi_asd =
			container_of(asd, struct cvi_isp_async_subdev, asd);
...
static int isp_subdev_notifier(struct cvi_isp_device *isp_dev)
{
	...
	ret = v4l2_async_notifier_parse_fwnode_endpoints(
		dev, ntf, sizeof(struct cvi_isp_async_subdev), cvi_isp_fwnode_parse);
...
static int cvi_isp_register_platform_subdevs(struct cvi_isp_device *dev)
{
	int ret;
...
static const struct media_device_ops cvi_isp_media_ops = {
	.link_notify = v4l2_pipeline_link_notify,
};


static int cvi_isp_plat_probe(struct platform_device *pdev)
{
	...
	if (!isp_dev) {
		printk("alloc isp dev mem fail !!\n");
		return -ENOMEM;
	}

	memset(&isp_dev->vi_dev, 0, sizeof(struct cvi_vi_dev));
	...
	isp_dev->media_dev.ops = &cvi_isp_media_ops;
	...
	ret = cvi_isp_register_platform_subdevs(isp_dev);
	if (ret < 0) {
...
static int cvi_isp_plat_remove(struct platform_device *pdev)
{
	struct v4l2_device *v4l2_dev = platform_get_drvdata(pdev);
	struct cvi_isp_device *isp_dev = container_of(v4l2_dev,
									struct cvi_isp_device, v4l2_dev);;
...
static const struct of_device_id cvi_isp_plat_of_match[] = {
	{
...
struct platform_driver cvi_isp_plat_drv = {
	.driver = {
		   .name = "ispv4l2",
		   .of_match_table = of_match_ptr(cvi_isp_plat_of_match),
	},
	.probe = cvi_isp_plat_probe,
	.remove = cvi_isp_plat_remove,
};

static int __init isp_drv_init(void)
{
	return platform_driver_register(&cvi_isp_plat_drv);
}

static void __exit isp_drv_exit(void)
{
	platform_driver_unregister(&cvi_isp_plat_drv);
}
