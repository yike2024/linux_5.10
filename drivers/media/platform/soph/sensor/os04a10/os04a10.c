...
static int start_streaming(struct os04a10 *os04a10)
{
	...
	if (g_sns_sync_info.num_of_regs > 3) {
		ret = os04a10_write_regs(os04a10, (struct os04a10_reg *)g_sns_sync_info.regs + 1,
			 ...
			 const struct i2c_device_id *devid)
{
	...
	if (index_id >= ARRAY_SIZE(supported_modes)) {
		...
		memcpy(os04a10->cur_mode, &supported_modes[0],
					sizeof(struct os04a10_mode));
	} else {
		os04a10->cur_mode = devm_kzalloc(&client->dev,
					sizeof(struct os04a10_mode), GFP_KERNEL);

		memcpy(os04a10->cur_mode, &supported_modes[index_id],
					sizeof(struct os04a10_mode));
	}
