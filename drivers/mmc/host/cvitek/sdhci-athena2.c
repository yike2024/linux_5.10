...
static void sdhci_athena2_emmc_setup_pad(struct sdhci_host *host)
{
}

static void sdhci_athena2_sd_setup_pad(struct sdhci_host *host)
{
...
static void sdhci_athena2_sd1_setup_pad(struct sdhci_host *host)
{
...
static void sdhci_athena2_sd_setup_io(struct sdhci_host *host, bool reset)
{
...
static void sdhci_athena2_sd1_setup_io(struct sdhci_host *host, bool reset)
{
...
static int sdhci_athena2_general_select_drive_strength(struct sdhci_host *host,
...
static void sdhci_cvi_athena2_set_tap(struct sdhci_host *host, unsigned int tap)
{
	pr_debug("%s %d\n", __func__, tap);
...
static int sdhci_athena2_general_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	u16 min = 0;
	u32 k = 0;
	s32 ret;
	...
	while (min < TUNE_MAX_PHCODE) {
		retry_cnt = 0;
		sdhci_cvi_athena2_set_tap(host, min);
...
retry_tuning:
	...
	sdhci_cvi_athena2_set_tap(host, final_tap);
...
static void sdhci_athena2_emmc_reset(struct sdhci_host *host, u8 mask)
{
	u16 ctrl_2;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
static void sdhci_athena2_sd_reset(struct sdhci_host *host, u8 mask)
{
	u16 ctrl_2;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
static void sdhci_athena2_sdio_reset(struct sdhci_host *host, u8 mask)
{
	u16 ctrl_2;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
void sdhci_athena2_sd_voltage_switch(struct sdhci_host *host)
{
	uint32_t reg = 0;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
void sdhci_athena2_sd1_voltage_switch(struct sdhci_host *host)
{
	uint32_t reg = 0;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
void sdhci_athena2_sd_voltage_restore(struct sdhci_host *host, bool bunplug)
{
...
void sdhci_athena2_sd1_voltage_restore(struct sdhci_host *host, bool bunplug)
{
...
static void sdhci_athena2_sd_set_power(struct sdhci_host *host, unsigned char mode,
				unsigned short vdd)
{
	struct mmc_host *mmc = host->mmc;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	...
	if (mode == MMC_POWER_ON && mmc->ops->get_cd(mmc)) {
		...
		if (!cvi_host->sd_voltage_1_8_v)
			sdhci_athena2_sd_voltage_restore(host, false);
		sdhci_athena2_sd_setup_pad(host);
		//sdhci_athena2_sd_setup_io(host, false);
	} else if (mode == MMC_POWER_OFF) {
		//sdhci_athena2_sd_setup_io(host, true);
		pr_debug("[%s]===> power off\n", __func__);
		sdhci_athena2_sd_voltage_restore(host, true);
		sdhci_set_power_noreg(host, mode, vdd);
		mdelay(30);
	}
...
static void sdhci_athena2_sd1_set_power(struct sdhci_host *host, unsigned char mode,
				unsigned short vdd)
{
	struct mmc_host *mmc = host->mmc;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	...
	if (mode == MMC_POWER_ON && mmc->ops->get_cd(mmc)) {
		sdhci_set_power_noreg(host, mode, vdd);
		if (!cvi_host->sd_voltage_1_8_v)
			sdhci_athena2_sd1_voltage_restore(host, false);
		sdhci_athena2_sd1_setup_pad(host);
		//sdhci_athena2_sd1_setup_io(host, false);
	} else if (mode == MMC_POWER_OFF) {
		sdhci_athena2_sd1_voltage_restore(host, true);
		sdhci_set_power_noreg(host, mode, vdd);
		mdelay(30);
	}
}

static void sdhci_athena2_emmc_dump_vendor_regs(struct sdhci_host *host)
{

}

static void sdhci_athena2_sd_dump_vendor_regs(struct sdhci_host *host)
{
	//struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	//struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
}
...
static const struct sdhci_ops sdhci_athena2_emmc_ops = {
	.reset = sdhci_athena2_emmc_reset,
	...
	.platform_execute_tuning = sdhci_athena2_general_execute_tuning,
	.select_drive_strength = sdhci_athena2_general_select_drive_strength,
	.dump_vendor_regs = sdhci_athena2_emmc_dump_vendor_regs,
	.adma_write_desc = cvi_adma_write_desc,
};

static const struct sdhci_ops sdhci_athena2_sd_ops = {
	.reset = sdhci_athena2_sd_reset,
	.set_clock = sdhci_set_clock,
	.set_power = sdhci_athena2_sd_set_power,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
	.voltage_switch = sdhci_athena2_sd_voltage_switch,
	.set_uhs_signaling = sdhci_cvi_general_set_uhs_signaling,
	.platform_execute_tuning = sdhci_athena2_general_execute_tuning,
	.select_drive_strength = sdhci_athena2_general_select_drive_strength,
	.dump_vendor_regs = sdhci_athena2_sd_dump_vendor_regs,
	.dump_vendor_regs = NULL,
	.adma_write_desc = cvi_adma_write_desc,
};

static const struct sdhci_ops sdhci_athena2_sd1_ops = {
	.reset = sdhci_athena2_sd_reset,
	.set_clock = sdhci_set_clock,
	.set_power = sdhci_athena2_sd1_set_power,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
	.voltage_switch = sdhci_athena2_sd1_voltage_switch,
	.set_uhs_signaling = sdhci_cvi_general_set_uhs_signaling,
	.platform_execute_tuning = sdhci_athena2_general_execute_tuning,
	.select_drive_strength = sdhci_athena2_general_select_drive_strength,
	.dump_vendor_regs = sdhci_athena2_sd_dump_vendor_regs,
...
static const struct sdhci_ops sdhci_athena2_sdio_ops = {
	.reset = sdhci_athena2_sdio_reset,
	.set_clock = sdhci_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
	.voltage_switch = sdhci_athena2_sd_voltage_switch,
...
static const struct sdhci_ops sdhci_athena2_fpga_emmc_ops = {
	.reset = sdhci_athena2_sd_reset,
	.set_clock = sdhci_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
...
static const struct sdhci_ops sdhci_athena2_fpga_sd_ops = {
	.reset = sdhci_athena2_sd_reset,
	.set_clock = sdhci_set_clock,
#ifndef CONFIG_ARCH_ATHENA2_FPGA
	.set_power = sdhci_athena2_sd_set_power,
#endif
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
...
static const struct sdhci_pltfm_data sdhci_athena2_emmc_pdata = {
	.ops = &sdhci_athena2_emmc_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN,
};

static const struct sdhci_pltfm_data sdhci_athena2_sd_pdata = {
	.ops = &sdhci_athena2_sd_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN,
};

static const struct sdhci_pltfm_data sdhci_athena2_sd1_pdata = {
	.ops = &sdhci_athena2_sd1_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN,
};

static const struct sdhci_pltfm_data sdhci_athena2_sdio_pdata = {
	.ops = &sdhci_athena2_sdio_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_NO_1_8_V,
};

static const struct sdhci_pltfm_data sdhci_athena2_fpga_emmc_pdata = {
	.ops = &sdhci_athena2_fpga_emmc_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_BROKEN_HS200,
};

static const struct sdhci_pltfm_data sdhci_athena2_fpga_sd_pdata = {
	.ops = &sdhci_athena2_fpga_sd_ops,
...
static const struct of_device_id sdhci_cvi_dt_match[] = {
	{.compatible = "cvitek,athena2-fpga-emmc", .data = &sdhci_athena2_fpga_emmc_pdata},
	{.compatible = "cvitek,athena2-fpga-sd", .data = &sdhci_athena2_fpga_sd_pdata},
	{.compatible = "cvitek,athena2-emmc", .data = &sdhci_athena2_emmc_pdata},
	{.compatible = "cvitek,athena2-sd", .data = &sdhci_athena2_sd_pdata},
	{.compatible = "cvitek,athena2-sd1", .data = &sdhci_athena2_sd1_pdata},
	{.compatible = "cvitek,athena2-sdio", .data = &sdhci_athena2_sdio_pdata},
...
static int sdhci_cvi_probe(struct platform_device *pdev)
{
	...
	if (strstr(dev_name(mmc_dev(host->mmc)), "cv-emmc"))
		sdhci_athena2_emmc_setup_pad(host);
