...
static void cvitek_lvds_encoder_enable(struct drm_encoder *encoder)
{
	...
	if (ret)
	{
		drm_err(lvds->drm,"drm_of_find_panel_or_bridge");
		return;
	}
...
struct lvds_match_data athena2_lvds0 = {
	.lvds_id = 0,
};

struct lvds_match_data athena2_lvds1 = {
	.lvds_id = 1,
};

static const struct of_device_id lvds_match_table[] = {
	{ .compatible = "cvitek,athena2_lvds0",
	  .data = &athena2_lvds0 },
#if IS_ENABLED(CONFIG_CVITEK_LVDS1)
	{ .compatible = "cvitek,athena2_lvds1",
	  .data = &athena2_lvds1 },
#endif
	{ /* end node */ },
};
