...
static void cvitek_drm_mode_config_init(struct drm_device *dev)
{
	...
	dev->mode_config.max_width = 4608;
...
static int cvitek_drm_bind(struct device *dev)
{
	...
	/* extend axi to 36 bit */
	cma_object = drm_gem_cma_create(drm, 1);
	if (IS_ERR(cma_object)) {
		drm_err(drm, "failed to alloc cma buffer for extending axi to 36 bit\n");
		goto err_unbind_all;
	}

	base_addr = cma_object->paddr;
	extern_axi_to_36bit(base_addr >> 32);
	drm_gem_cma_free_object(&cma_object->base);
...
MODULE_DESCRIPTION("cvitek Athena2 SoCs' DRM master driver");
