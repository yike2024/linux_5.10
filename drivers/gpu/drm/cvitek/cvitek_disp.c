			       ...
			       struct drm_crtc_state *old_state)
{
	...
	struct drm_color_lut *lut = (struct drm_color_lut *)crtc->state->gamma_lut->data;
			       ...
			       u32 src_y, u32 src_w, u32 src_h)
{
	...
	if (type == DRM_PLANE_TYPE_PRIMARY) {
		...
		if (crtc_x < 0) {
			...
			ctx->disp_cfg.gop_cfg.ow_cfg[0].start.x = crtc_x;
			ctx->disp_cfg.gop_cfg.ow_cfg[0].img_size.w = ctx->disp_timing.width - crtc_x;
		} else {
			ctx->disp_cfg.gop_cfg.ow_cfg[0].start.x = crtc_x;
			ctx->disp_cfg.gop_cfg.ow_cfg[0].img_size.w = crtc_w;
		}
				     ...
				     struct drm_plane_state *old_state)
{
	...
	if(plane->type == DRM_PLANE_TYPE_PRIMARY) {
		...
		ctx->disp_cfg.gop_cfg.gop_ctrl.b.ow0_en = 0;
		disp_gop_set_cfg(ctx, ch - 1, &ctx->disp_cfg.gop_cfg);
	}
}
...
struct disp_match_data athena2_disp0 = {
	.crtc_id = 0,
	.num_planes = 4,
	.prim_plane = 0,
	.gamma_lut_size = 65,
...
struct disp_match_data athena2_disp1 = {
	.crtc_id = 1,
	.num_planes = 4,
	.prim_plane = 0,
	.gamma_lut_size = 65,
