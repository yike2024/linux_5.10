...
struct athena2_thermal_zone {
    unsigned int ch;
    void __iomem *base;
    struct athena2_thermal *ct;
};

struct athena2_thermal {
    struct device *dev;
    void __iomem *base;
...
static void athena2_thermal_init(struct athena2_thermal *ct)
{
...
static void athena2_thermal_uninit(struct athena2_thermal *ct)
{
    void __iomem *base = ct->base;
    u32 regval;
...
static int athena2_read_temp(void *data, int *temperature)
{
	struct athena2_thermal_zone *ctz = data;
...
static const struct thermal_zone_of_device_ops athena2_thermal_ops = {
    .get_temp = athena2_read_temp,
};

static const struct of_device_id athena2_thermal_of_match[] = {
    {
        .compatible = "cvitek,athena2-thermal",
    },
    {},
};
MODULE_DEVICE_TABLE(of, athena2_thermal_of_match);

static int athena2_thermal_probe(struct platform_device *pdev)
{
    struct athena2_thermal *ct;
    struct athena2_thermal_zone *ctz;
    struct resource *res;
...
static int athena2_thermal_remove(struct platform_device *pdev)
{
    struct athena2_thermal *ct = platform_get_drvdata(pdev);

    athena2_thermal_uninit(ct);
...
static int athena2_thermal_suspend(struct device *dev)
{
    struct athena2_thermal *ct = dev_get_drvdata(dev);
...
static int athena2_thermal_resume(struct device *dev)
{
    struct athena2_thermal *ct = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(athena2_thermal_pm_ops, athena2_thermal_suspend,
                         athena2_thermal_resume);

static struct platform_driver athena2_thermal_driver = {
    .probe = athena2_thermal_probe,
    .remove = athena2_thermal_remove,
    .driver =
        {
            .name = "athena2-thermal",
            .pm = &athena2_thermal_pm_ops,
            .of_match_table = athena2_thermal_of_match,
        },
};

module_platform_driver(athena2_thermal_driver);
MODULE_DESCRIPTION("athena2 thermal driver");
MODULE_LICENSE("GPL");
