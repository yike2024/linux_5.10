...
#include <dt-bindings/clock/athena2-clock.h>

//#define ATHENA2_CLK_FLAGS_ALL	(CLK_GET_RATE_NOCACHE)
// #define ATHENA2_CLK_FLAGS_ALL	(CLK_GET_RATE_NOCACHE | CLK_IS_CRITICAL)
#define ATHENA2_CLK_FLAGS_ALL	(CLK_GET_RATE_NOCACHE | CLK_IGNORE_UNUSED)
#define ATHENA2_CLK_FLAGS_MUX	(CLK_SET_RATE_PARENT | CLK_SET_RATE_NO_REPARENT)
...
#define ATHENA2_PLL_LOCK_TIMEOUT_MS	200
...
#define to_athena2_pll_clk(_hw) container_of(_hw, struct athena2_pll_hw_clock, hw)
#define to_athena2_clk(_hw) container_of(_hw, struct athena2_hw_clock, hw)

#define div_mask(width) ((1 << (width)) - 1)

static DEFINE_SPINLOCK(athena2_clk_lock);

struct athena2_clock_data {
	void __iomem *base;
	spinlock_t *lock;
...
struct athena2_hw_clock {
	...
	struct athena2_gate gate;
	struct athena2_div div[2]; /* 0: DIV_IN0, 1: DIV_IN1 */
	struct athena2_mux mux[3]; /* 0: bypass, 1: CLK_SEL, 2: CLK_SRC(DIV_IN0_SRC_MUX) */
};

struct athena2_pll_clock {
	unsigned int	id;
	const char	*name;
	u32		reg_csr;
	u32		reg_ssc;
...
struct athena2_pll_hw_clock {
	struct athena2_pll_clock pll;
	void __iomem *base;
	spinlock_t *lock;
	struct clk_hw hw;
};

static const struct clk_ops athena2_g6_pll_ops;
static const struct clk_ops athena2_g2_pll_ops;
static const struct clk_ops athena2_g2d_pll_ops;
static const struct clk_ops athena2_clk_ops;

static struct athena2_clock_data *clk_data;

static unsigned long cvi_clk_flags;

#define ATHENA2_CLK(_id, _name, _parents, _gate_reg, _gate_shift,		\
...
const char *const athena2_pll_parent[] = {"osc"};
const char *const athena2_frac_pll_parent[] = {"clk_mipimpll2"};

/*
 * All PLL clocks are marked as CRITICAL, hence they are very crucial
 * for the functioning of the SoC
 */
static struct athena2_pll_hw_clock athena2_pll_clks[] = {
	CLK_G6_PLL(ATHENA2_CLK_FPLL, "clk_fpll", athena2_pll_parent, REG_FPLL_CSR, 0),
	CLK_G6_PLL(ATHENA2_CLK_MPLL, "clk_mpll", athena2_pll_parent, REG_MPLL_CSR, 0),
	CLK_G6_PLL(ATHENA2_CLK_TPLL, "clk_tpll", athena2_pll_parent, REG_TPLL_CSR, 0),
	CLK_G2_PLL(ATHENA2_CLK_MIPIMPLL0, "clk_mipimpll0", athena2_pll_parent,
			REG_MIPIMPLL0_CSR, 0, 0),
	CLK_G2_PLL(ATHENA2_CLK_CAM0PLL, "clk_cam0pll", athena2_pll_parent,
			REG_CAM0PLL_CSR, 0, CLK_IGNORE_UNUSED),
	CLK_G2_PLL(ATHENA2_CLK_DISPPLL0, "clk_disppll0", ((const char *[]) {"clk_mipimpll0"}),
			REG_DISPPLL0_CSR, REG_DISPPLL0_SSC_SYN_CTRL, 0),
	CLK_G2_PLL(ATHENA2_CLK_MIPIMPLL1, "clk_mipimpll1", athena2_pll_parent,
			REG_MIPIMPLL1_CSR, 0, 0),
	CLK_G2_PLL(ATHENA2_CLK_CAM1PLL, "clk_cam1pll", athena2_pll_parent,
			REG_CAM1PLL_CSR, 0, 0),
	CLK_G2_PLL(ATHENA2_CLK_DISPPLL1, "clk_disppll1", ((const char *[]) {"clk_mipimpll1"}),
			REG_DISPPLL1_CSR, REG_DISPPLL1_SSC_SYN_CTRL, 0),
	CLK_G2_PLL(ATHENA2_CLK_MIPIMPLL2, "clk_mipimpll2", athena2_pll_parent,
			REG_MIPIMPLL2_CSR, 0, 0),
	CLK_G2_PLL(ATHENA2_CLK_A0PLL, "clk_a0pll", ((const char *[]) {"clk_mipimpll2"}),
			REG_APLL0_CSR, REG_APLL0_SSC_SYN_CTRL, 0),
	CLK_INT_N_PLL(ATHENA2_CLK_A53PLL, "clk_a53pll", athena2_pll_parent, REG_A53PLL_CSR, 0),
	CLK_INT_N_PLL(ATHENA2_CLK_RVPLL, "clk_rvpll", athena2_pll_parent, REG_RVPLL_CSR, 0)
};
 ...
 * of the SoC.
 */
static struct athena2_hw_clock athena2_clks[] = {
	ATHENA2_CLK(ATHENA2_TOP_CLK_FAB0, "clk_fab0",
	...
	ATHENA2_CLK(ATHENA2_AP_CLK_RV0, "ap_clk_rv0",
	...
	ATHENA2_CLK(ATHENA2_VD0_CLK_SRC0_VDSYS, "vd0_clk_src0_vdsys",
	...
	ATHENA2_CLK(ATHENA2_VI_CLK_SRC_VI_SYS_1, "vi_clk_src_vi_sys_1",
			((const char *[]) { "osc", "clk_mipimpll0", "clk_mipimpll2", "clk_mipimpll1"}),
			REG_CLK_EN_1, 12,
			REG_DIV_VI_CLK_SRC_VI_SYS_1, 6, 0xff, 0,
			REG_CLK_BYP_1, 12, 0, -1,
			REG_DIV_VI_CLK_SRC_VI_SYS_1, 0),
	ATHENA2_CLK(ATHENA2_VI_CLK_SRC_VI_SYS_2, "vi_clk_src_vi_sys_2",
	...
	ATHENA2_CLK(ATHENA2_VI_CLK_SRC_VI_SYS_3, "vi_clk_src_vi_sys_3",
	...
	ATHENA2_CLK(ATHENA2_VI_CLK_SRC_VI_SYS_4, "vi_clk_src_vi_sys_4",
			((const char *[]) { "osc", "clk_mipimpll0", "clk_mipimpll2", "clk_mipimpll1"}),
			REG_CLK_EN_1, 15,
			REG_DIV_VI_CLK_SRC_VI_SYS_4, 2, 0xff, 0,
			REG_CLK_BYP_1, 15, 0, -1,
			REG_DIV_VI_CLK_SRC_VI_SYS_4, 0),
	ATHENA2_CLK(ATHENA2_VI_CLK_SRC_VI_SYS_5, "vi_clk_src_vi_sys_5",
	...
	ATHENA2_CLK(ATHENA2_VO_CLK_RAW_AXI, "vo_clk_raw_axi",
			((const char *[]) { "osc", "clk_cam0pll", "clk_mipimpll2", "clk_mipimpll1"}),
			REG_CLK_EN_1, 17,
			REG_DIV_VO_CLK_RAW_AXI, 2, 0xff, 0,
			REG_CLK_BYP_1, 17, 0, -1,
			REG_DIV_VO_CLK_RAW_AXI, 0),
	ATHENA2_CLK(ATHENA2_VO_CLK_X2P, "vo_clk_x2p",
			((const char *[]) { "osc", "clk_fab1"}),
			REG_CLK_EN_1, 18, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_1, 18, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_VO_CLK_SRC_VO_SYS_0, "clk_src_vo_sys_0",
	...
	ATHENA2_CLK(ATHENA2_VO_CLK_SRC_VO_SYS_1, "clk_src_vo_sys_1",
	...
	ATHENA2_CLK(ATHENA2_VO_CLK_SRC_VO_SYS_2, "clk_src_vo_sys_2",
	...
	ATHENA2_CLK(ATHENA2_VO_CLK_SYS_DISP0, "vo_clk_sys_disp0",
	...
	ATHENA2_CLK(ATHENA2_VO_CLK_SYS_DISP1, "vo_clk_sys_disp1",
	...
	ATHENA2_CLK(ATHENA2_DDR_CLKI_DDRPLL_MAS_REF_SYS1, "ddr_clki_ddrpll_mas_ref_sys1",
			((const char *[]) { "osc"}),
			REG_CLK_EN_1, 30, 0xff, 0, 0xff, 0, 0, -1, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_CLKI_DDRPLL_MAS_REF_SYS2, "ddr_clki_ddrpll_mas_ref_sys2",
			((const char *[]) { "osc"}),
			REG_CLK_EN_1, 31, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_1, 31, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_ACLK_M1_SYS1, "ddr_aclk_m1_sys1",
			((const char *[]) { "osc", "vi_clk_raw_axi"}),
			REG_CLK_EN_2, 0, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 0, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_ACLK_M1_SYS2, "ddr_aclk_m1_sys2",
			((const char *[]) { "osc", "vi_clk_raw_axi"}),
			REG_CLK_EN_2, 1, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 1, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_ACLK_M4_SYS1, "ddr_aclk_m4_sys1",
			((const char *[]) { "osc", "tpu_clk_tpu"}),
			REG_CLK_EN_2, 2, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 2, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_ACLK_M4_SYS2, "ddr_aclk_m4_sys2",
			((const char *[]) { "osc", "tpu_clk_tpu"}),
			REG_CLK_EN_2, 3, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 3, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_PCLK_CTRL_SYS1, "ddr_pclk_ctrl_sys1",
			((const char *[]) { "osc", "clk_fab1"}),
			REG_CLK_EN_2, 4, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 4, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_PCLK_CTRL_SYS2, "ddr_pclk_ctrl_sys2",
			((const char *[]) { "osc", "clk_fab1"}),
			REG_CLK_EN_2, 5, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 5, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE, "ddr_top_fab_aclk_offline",
			((const char *[]) { "osc", "ddr_aclk_m2_sys1"}),
			REG_CLK_EN_2, 6, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 6, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_RLT, "ddr_top_fab_aclk_rlt",
			((const char *[]) { "osc", "vi_clk_raw_axi"}),
			REG_CLK_EN_2, 7, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 7, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_TPU, "ddr_top_fab_aclk_tpu",
			((const char *[]) { "osc", "tpu_clk_tpu"}),
			REG_CLK_EN_2, 8, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 8, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M1, "ddr_top_fab_aclk_offline_m1",
			((const char *[]) { "osc", "vi_clk_raw_axi"}),
			REG_CLK_EN_2, 9, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 9, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M2, "ddr_top_fab_aclk_offline_m2",
			((const char *[]) { "osc", "vi_clk_raw_axi"}),
			REG_CLK_EN_2, 10, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 10, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M3, "ddr_top_fab_aclk_offline_m3",
			((const char *[]) { "osc", "vi_clk_raw_axi"}),
			REG_CLK_EN_2, 11, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 11, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M4, "ddr_top_fab_aclk_offline_m4",
			((const char *[]) { "osc", "vd0_clk_axi_vdsys_0"}),
			REG_CLK_EN_2, 12, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 12, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M5, "ddr_top_fab_aclk_offline_m5",
			((const char *[]) { "osc", "vd0_clk_axi_vdsys_1"}),
			REG_CLK_EN_2, 13, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 13, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M6, "ddr_top_fab_aclk_offline_m6",
			((const char *[]) { "osc", "vd1_clk_axi_vdsys_0"}),
			REG_CLK_EN_2, 14, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 14, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M7, "ddr_top_fab_aclk_offline_m7",
			((const char *[]) { "osc", "vd1_clk_axi_vdsys_1"}),
			REG_CLK_EN_2, 15, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 15, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M8, "ddr_top_fab_aclk_offline_m8",
			((const char *[]) { "osc", "ve_clk_axi_vesys"}),
			REG_CLK_EN_2, 16, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 16, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M9, "ddr_top_fab_aclk_offline_m9",
			((const char *[]) { "osc", "clk_axi4"}),
			REG_CLK_EN_2, 17, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 17, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M10, "ddr_top_fab_aclk_offline_m10",
			((const char *[]) { "osc", "usb_clk_usb_fab"}),
			REG_CLK_EN_2, 18, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 18, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M11, "ddr_top_fab_aclk_offline_m11",
			((const char *[]) { "osc", "ssperi_aclk_ddr"}),
			REG_CLK_EN_2, 19, 0xff, 0, 0xff, 0,
			REG_CLK_BYP_2, 19, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_ACLK_OFFLINE_M12, "ddr_top_fab_aclk_offline_m12",
			((const char *[]) { "osc", "ap_bus_clk"}),
			REG_CLK_EN_2, 20, 0xff, 1, 0xff, 0,
			REG_CLK_BYP_2, 20, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_PCLK_FW, "ddr_top_fab_pclk_fw",
			((const char *[]) { "osc", "clk_security"}),
			REG_CLK_EN_2, 21, 0xff, 1, 0xff, 0,
			REG_CLK_BYP_2, 21, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_DDR_TOP_FAB_PCLK_TOP, "ddr_top_fab_pclk_top",
			((const char *[]) { "osc", "clk_fab1"}),
			REG_CLK_EN_2, 22, 0xff, 1, 0xff, 0,
			REG_CLK_BYP_2, 22, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_USB_CLK_TOP_USB_FAB, "usb_clk_top_usb_fab",
			((const char *[]) { "osc", "clk_fab1"}),
			REG_CLK_EN_2, 23, 0xff, 1, 0xff, 0,
			REG_CLK_BYP_2, 23, 0, -1, 0xff, 0),
	ATHENA2_CLK(ATHENA2_USB_TEST_PHY_CLK_480, "usb_test_phy_clk_480",
...
static unsigned long athena2_pll_rate_calc(u32 regval, s16 post_div_sel, unsigned long parent_rate)
{
	u64 numerator;
...
static unsigned long athena2_g6_pll_recalc_rate(struct clk_hw *hw,
		unsigned long parent_rate)
{
	struct athena2_pll_hw_clock *pll_hw = to_athena2_pll_clk(hw);
	unsigned long rate;
	u32 regval;

	regval = readl(pll_hw->base + pll_hw->pll.reg_csr);
	rate = athena2_pll_rate_calc(regval, pll_hw->pll.post_div_sel, parent_rate);

	return rate;
}

static long athena2_g6_pll_round_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long *prate)
{
	return rate;
}

static int athena2_g6_pll_calc_csr(unsigned long parent_rate, unsigned long rate, u32 *csr)
{
	u64 numerator;
	u32 denominator;
...
static int athena2_g6_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				  unsigned long parent_rate)
{
	struct athena2_pll_hw_clock *pll_hw = to_athena2_pll_clk(hw);
	...
	ret = athena2_g6_pll_calc_csr(parent_rate, rate, &regval_csr);
	if (ret < 0)
		return ret;
	...
	timeout = ktime_add_ms(ktime_get(), ATHENA2_PLL_LOCK_TIMEOUT_MS);
	while (1) {
...
static const struct clk_ops athena2_g6_pll_ops = {
	.recalc_rate = athena2_g6_pll_recalc_rate,
	.round_rate = athena2_g6_pll_round_rate,
	.set_rate = athena2_g6_pll_set_rate,
};

static unsigned long athena2_g2_pll_recalc_rate(struct clk_hw *hw,
					       unsigned long parent_rate)
{
	struct athena2_pll_hw_clock *pll_hw = to_athena2_pll_clk(hw);
	u32 reg_ssc_set;
...
rate_calc:
	rate = athena2_pll_rate_calc(regval_csr, pll_hw->pll.post_div_sel, clk_ref);
...
static int athena2_g2_pll_get_setting_from_lut(unsigned long rate, u32 *csr,
					      u32 *ssc_set)
{
	int i;
...
static long athena2_g2_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				     unsigned long *prate)
{
	return rate;
}

static int athena2_g2_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				  unsigned long parent_rate)
{
	struct athena2_pll_hw_clock *pll_hw = to_athena2_pll_clk(hw);
	unsigned long flags = 0;
	...
	ret = athena2_g2_pll_get_setting_from_lut(rate, &regval_csr,
						 &regval_ssc_set);
	if (ret < 0)
		return ret;
...
static const struct clk_ops athena2_g2_pll_ops = {
	.recalc_rate = athena2_g2_pll_recalc_rate,
	.round_rate = athena2_g2_pll_round_rate,
	.set_rate = athena2_g2_pll_set_rate,
};
...
static struct clk_hw *athena2_clk_register_pll(struct athena2_pll_hw_clock *pll_clk,
...
static void athena2_clk_unregister_pll(struct clk_hw *hw)
{
	struct athena2_pll_hw_clock *pll_hw = to_athena2_pll_clk(hw);
...
static int athena2_clk_register_plls(struct athena2_pll_hw_clock *clks,
				    int num_clks,
				    struct athena2_clock_data *data)
{
	struct clk_hw *hw;
	...
	for (i = 0; i < num_clks; i++) {
		struct athena2_pll_hw_clock *athena2_clk = &clks[i];

		hw = athena2_clk_register_pll(athena2_clk, pll_base);
		if (IS_ERR(hw)) {
			pr_err("%s: failed to register clock %s\n",
			       __func__, athena2_clk->pll.name);
			goto err_clk;
		}

		data->hw_data.hws[clks[i].pll.id] = hw;

		clk_hw_register_clkdev(hw, athena2_clk->pll.name, NULL);
	}
...
err_clk:
	while (i--)
		athena2_clk_unregister_pll(data->hw_data.hws[clks[i].pll.id]);

	return PTR_ERR(hw);
}

static int athena2_clk_is_bypassed(struct athena2_hw_clock *clk_hw)
{
	u32 val;
...
static int athena2_clk_get_clk_sel(struct athena2_hw_clock *clk_hw)
{
	u32 val;
...
static int athena2_clk_get_src_sel(struct athena2_hw_clock *clk_hw)
{
	u32 val;
...
static unsigned long athena2_clk_div_recalc_rate(struct clk_hw *hw,
					      unsigned long parent_rate)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
	unsigned int clk_sel = athena2_clk_get_clk_sel(clk_hw);
	...
	if ((clk_hw->mux[0].shift >= 0) && athena2_clk_is_bypassed(clk_hw))
		return parent_rate;
...
static long athena2_clk_div_round_rate(struct clk_hw *hw, unsigned long rate,
				      unsigned long *prate)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
	unsigned int clk_sel = athena2_clk_get_clk_sel(clk_hw);
...
static long athena2_clk_div_calc_round_rate(struct clk_hw *hw, unsigned long rate,
					 unsigned long *prate)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
...
static int athena2_clk_div_determine_rate(struct clk_hw *hw,
				       struct clk_rate_request *req)
{
	...
	if ((parent_count < 2) || (clk_hw_get_flags(hw) & CLK_SET_RATE_NO_REPARENT)) {
		rate = athena2_clk_div_round_rate(hw, req->rate, &req->best_parent_rate);
		if (rate < 0)
			return rate;
	...
	best_rate = athena2_clk_div_calc_round_rate(hw, req->rate, &parent_rate);
	...
	for (which = 0; which < parent_count; which++) {
		...
		other_rate = athena2_clk_div_calc_round_rate(hw, req->rate, &parent_rate);
...
static int athena2_clk_div_set_rate(struct clk_hw *hw, unsigned long rate,
				 unsigned long parent_rate)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
	unsigned int clk_sel = athena2_clk_get_clk_sel(clk_hw);
...
static void athena2_clk_gate_endisable(struct clk_hw *hw, int enable)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
...
static int athena2_clk_gate_enable(struct clk_hw *hw)
{
	athena2_clk_gate_endisable(hw, 1);

	return 0;
}

static void athena2_clk_gate_disable(struct clk_hw *hw)
{
	athena2_clk_gate_endisable(hw, 0);
}

static int athena2_clk_gate_is_enabled(struct clk_hw *hw)
{
	u32 reg;
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
	void __iomem *reg_addr = clk_hw->base + clk_hw->gate.reg;
...
static u8 athena2_clk_mux_get_parent(struct clk_hw *hw)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
	u8 clk_sel = athena2_clk_get_clk_sel(clk_hw);
	u8 src_sel = athena2_clk_get_src_sel(clk_hw);
	u8 parent_idx = 0;
	...
	if (clk_hw->mux[0].shift >= 0) {
		// clk with bypass reg
		if (athena2_clk_is_bypassed(clk_hw)) {
			parent_idx = 0;
		} else {
			if (clk_hw->mux[1].shift >= 0) {
...
static int athena2_clk_mux_set_parent(struct clk_hw *hw, u8 index)
{
	struct athena2_hw_clock *clk_hw = to_athena2_clk(hw);
	unsigned long flags = 0;
...
static const struct clk_ops athena2_clk_ops = {
	// gate
	.enable = athena2_clk_gate_enable,
	.disable = athena2_clk_gate_disable,
	.is_enabled = athena2_clk_gate_is_enabled,

	// div
	.recalc_rate = athena2_clk_div_recalc_rate,
	.round_rate = athena2_clk_div_round_rate,
	.determine_rate = athena2_clk_div_determine_rate,
	.set_rate = athena2_clk_div_set_rate,

	//mux
	.get_parent = athena2_clk_mux_get_parent,
	.set_parent = athena2_clk_mux_set_parent,
};

static struct clk_hw *athena2_register_clk(struct athena2_hw_clock *athena2_clk,
					  void __iomem *sys_base)
{
	struct clk_hw *hw;
	struct clk_init_data init;
	...
	athena2_clk->gate.flags = 0;
	athena2_clk->div[0].flags = CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO |
				 CLK_DIVIDER_ROUND_CLOSEST;
	athena2_clk->div[1].flags = CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO |
				 CLK_DIVIDER_ROUND_CLOSEST;
	athena2_clk->mux[0].flags = 0; /* clk byp */
	athena2_clk->mux[1].flags = 0; /* clk sel */
	athena2_clk->mux[2].flags = 0; /* div_0 src_sel */
	athena2_clk->base = sys_base;
	athena2_clk->lock = &athena2_clk_lock;

	if (cvi_clk_flags) {
		/* copy clk_init_data for modification */
		memcpy(&init, athena2_clk->hw.init, sizeof(init));

		init.flags |= cvi_clk_flags;
		athena2_clk->hw.init = &init;
	}
...
static void athena2_unregister_clk(struct clk_hw *hw)
{
	struct athena2_hw_clock *athena2_clk = to_athena2_clk(hw);

	clk_hw_unregister(hw);
	kfree(athena2_clk);
}
static int athena2_register_clks(struct athena2_hw_clock *clks,
				int num_clks,
				struct athena2_clock_data *data)
{
	struct clk_hw *hw;
	...
	for (i = 0; i < num_clks; i++) {
		struct athena2_hw_clock *athena2_clk = &clks[i];

		//pr_err("%s: register clock %s\n", __func__, athena2_clk->name);
		hw = athena2_register_clk(athena2_clk, sys_base);
		...
		clk_hw_register_clkdev(hw, athena2_clk->name, NULL);
	}
...
err_clk:
	while (i--)
		athena2_unregister_clk(data->hw_data.hws[clks[i].id]);
...
static void __init cvi_clk_init(struct device_node *node)
{
	...
	num_clks = ARRAY_SIZE(athena2_pll_clks) +
		   ARRAY_SIZE(athena2_clks);

	clk_data = kzalloc(sizeof(struct athena2_clock_data) +
	...
	clk_data->lock = &athena2_clk_lock;

	clk_data->base = of_iomap(node, 0);
	if (!clk_data->base) {
		pr_err("Failed to map address range for cvitek,athena2-clk node\n");
		return;
	}

	athena2_clk_register_plls(athena2_pll_clks,
			       ARRAY_SIZE(athena2_pll_clks),
			       clk_data);
	athena2_register_clks(athena2_clks,
			   ARRAY_SIZE(athena2_clks),
			   clk_data);
	...
	/* force enable clocks */
	// clk_prepare_enable(clk_data->hw_data.hws[ATHENA2_CLK_DSI_MAC_VIP]->clk);
	// clk_prepare_enable(clk_data->hw_data.hws[ATHENA2_CLK_DISP_VIP]->clk);
	// clk_prepare_enable(clk_data->hw_data.hws[ATHENA2_CLK_BT_VIP]->clk);
	// clk_prepare_enable(clk_data->hw_data.hws[ATHENA2_CLK_SC_TOP_VIP]->clk);
...
CLK_OF_DECLARE(cvi_clk, "cvitek,athena2-clk", cvi_clk_init);
