...
static int cvitek_pinctrl_suspend(struct device *dev)
{
	...
	for (i = 0; i < 12; ++i)
		memcpy_fromio(pinctrl.regs[i]->saved_regs,
			      pinctrl->regs[i].base, pintrl->regs[i].regs_size);
...
static int cvitek_pinctrl_resume(struct device *dev)
{
	...
	for (i = 0; i < 12; ++i)
		memcpy_toio(pinctrl->regs[i].base, pinctrl->regs[i].saved_regs,
...
static const struct of_device_id cvi_pinctrl_of_match[] = {
	{ .compatible = "cvitek,pinctrl-athena2" },
	{},
};
